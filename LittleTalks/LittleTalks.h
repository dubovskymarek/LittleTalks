...
///    \brief <B>deviceId</B> - source device, that is publishing topic. If device is 0, then topic is published via remote server.\n
...
/// \param toExpectLocal device is expect on local network
///
void LT_AddDevice(LT_DEVICE_ID deviceId, BOOL toExpectLocal);

///
/// \brief LT_IsSomeDeviceRemoted
///
/// \param deviceFlag map of devices, who are checkek
///
/// \return true if some device is not connected on local network, because it is remoted on server
///
